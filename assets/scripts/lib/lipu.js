    ...
    matches.forEach(m => {
      ...
      if (k === "l") {
        ...
        if( target.length ) {
          ...
          line = line.replace(m[0], `<a href="/${url}" onclick="return linkClick(event);">${txt}</a>`);
        }
        ...
        for (let x in mi[m[2]]) {
          ...
          if (x !== "index") {
            ...
            list += `
              <div class="post">
                <span>
                  <a href="/${page.LINK}" onclick="return linkClick(event);">${page.NAME}</a><br>
